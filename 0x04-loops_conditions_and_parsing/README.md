...
Now that you’ve parsed the [Apache log file](./apache-access.log), let’s sort the data so you can get a better idea of what is going on.

Using what you did in the [previous exercise](./102-lets_parse_apache_logs), write a Bash script that groups visitors by `IP` and `HTTP` status code, and displays this data.
